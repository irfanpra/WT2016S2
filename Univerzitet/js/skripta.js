var novosti = [
		...
		new novost(8,
				   new Date(2016, 3, 15),
				   "Dodijeljeno 95 stipendija iz Fonda „Suada Dilberović“ studentima Univerziteta u Sarajevu",
...
var glavnaNovost = novosti[0];

function popuniNovosti(id) {
	glavnaNovost = novosti.find(function (nov) { return nov.ID == id; });
...
function novost(id, datum, naslov, podnaslov, slika, ikona, paragrafi) {
	this.ID = id;
...
function filtrirajNovosti(filter) {
	...
	for(i = 0; i < filterNovosti.length; i++) {
			maleNovosti.innerHTML += "<div onclick='popuniNovosti("+ filterNovosti[i].ID + ")' class='floating-vijest'><img src='" + filterNovosti[i].ikona + "'" + " alt='malavijest'><h3>" + filterNovosti[i].naslov + "</h3></div>";
	}
}
